...
class QBDraftPredictor:
    ...
    Evaluation strategy:
      - Primary benchmark : Grouped K-Fold (GKF) — years grouped
        chronologically into k blocks, each block held out once.
        Respects temporal order and provides stable F1 estimates.
      - Stability check   : Stratified K-Fold (SKF) — random stratified
        split, used to detect temporal leakage and confirm variance.

    If GKF ≈ SKF on mean F1, leakage is minimal and SKF variance gives
    a reliable lower bound on real-world uncertainty.
    If SKF >> GKF, temporal leakage is inflating SKF — trust GKF only.
    """
    ...
    def _objective(self, trial, X_train, y_train) -> float:
        """
        Optuna objective: maximise AUCPR via internal 5-fold CV.
        AUCPR is preferred over logloss for imbalanced data.
        """
        neg = (y_train == 0).sum()
        pos = (y_train == 1).sum()
    ...
    def hyperparameter_tuning(self, X_train, y_train, n_trials: int = 50) -> dict:
        """Run Optuna study and store best params."""
        print("\n" + "=" * 60)
    ...
    def _run_single_fold(self, X_train_fold, y_train_fold,
        ...
        """
        Train XGBoost on one fold, tune classification threshold, and
        compute all evaluation metrics.

        val_X / val_y: optional validation set for threshold tuning.
        Falls back to a 20% stratified split of the training data.
        """
        ...
        # Threshold tuning: prefer an explicit validation set; fall back to
        # a stratified 20% split carved from the training fold.
    ...
    def train_and_evaluate_grouped(self, X, y, df, n_splits: int = 5):
        ...
        Years are sorted chronologically and assigned to k roughly equal
        groups. Each group (2-3 consecutive years) is held out once as
        the test set, with all remaining years used for training.

        Why this is the primary benchmark:
          - Respects temporal ordering: no future data bleeds into training.
          - Groups of 2-3 years give each fold 6-11 first-round QBs,
            which is enough to compute a stable, meaningful F1 score.
          - Variance is much lower than single-year LOGO.

        Args:
            X:        feature DataFrame
            y:        target Series
            df:       full DataFrame (must contain 'year' column)
            n_splits: number of groups/folds (default 5)
        """
        print("\n" + "=" * 60)
        ...
        for fold_id in range(n_splits):
            ...
            # Use the most recent training group as the threshold-tuning
    ...
    def train_and_evaluate_skf(self, X, y, n_splits: int = 5):
        ...
        the 27% first-round QB ratio). Used to:
          1. Confirm that GKF variance is not just noise from fold size.
          2. Detect temporal leakage: if SKF F1 >> GKF F1, future data
             is leaking into GKF training, and GKF is the honest number.
          3. Provide a tighter confidence interval on model skill when
             leakage is shown to be minimal (SKF ≈ GKF scenario).

        Note: SKF is NOT the primary benchmark because its random splits
        allow QBs from later years to appear in training while QBs from
        earlier years appear in the test set, violating temporal order.
    ...
    def print_cross_validation_summary(self):
        """Print per-fold and aggregate metrics for the last CV run."""
        label_map = {
    ...
    def feature_importance_analysis(self):
        """
        Aggregate feature importance (by split count) across all folds
        and print the top 20. Should be called after GKF so importance
        reflects the primary benchmark models.
        """
        print("\n" + "=" * 60)
    ...
    def generate_predictions_for_new_qbs(self, new_qbs_csv: str = None):
        """
        Generate predictions for a new QB class using the last trained
        GKF model (most recent fold = most training data seen).
        """
        print("\n" + "=" * 60)
...
def main():
    """Demonstrate GKF (primary) followed by SKF (stability check)."""
